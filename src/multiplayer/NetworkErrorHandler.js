...
const logger = Logger.create('NetworkErrorHandler');
...
class NetworkErrorHandler {
    ...
    initialize() {
        ...
        this.reconnectionManager.on('reconnectAbandoned', (data) =>
            this.handleReconnectAbandoned(data)
        );
        this.reconnectionManager.on('connectionQualityChanged', (data) =>
            this.handleConnectionQualityChanged(data)
        );
    ...
    handleDisconnect(data) {
        logger.warn(`Disconnected: ${data.reason}`);
    ...
    handleReconnectAbandoned(data) {
        logger.warn('Reconnection abandoned');
    ...
    handleConnectionQualityChanged(data) {
        logger.info(`Connection quality changed: ${data.currentQuality}`);
    ...
    getConnectionStats() {
        ...
        const packetLossRate =
            this.totalPackets > 0 ? (this.packetLossCount / this.totalPackets) * 100 : 0;
