...
class DifficultyManager {
    ...
    constructor(game, aiController) {
        ...
        if (!this.validateAllConfigurations()) {
            console.warn('Difficulty system initialized with invalid configurations');
        }
        
        // Validate constructor parameters
        if (game && typeof game !== 'object') {
            console.warn('Invalid game parameter provided to DifficultyManager');
        }
        
        if (aiController && typeof aiController !== 'object') {
            console.warn('Invalid aiController parameter provided to DifficultyManager');
        }
    ...
    getDifficultyConfig(level = null) {
        ...
        if (level !== null) {
            if (typeof level !== 'string') {
                console.warn(`Invalid difficulty type in getDifficultyConfig: ${typeof level}, using current difficulty`);
        ...
        if (!config) {
            console.warn(`Configuration not found for difficulty "${targetLevel}", using medium`);
            return DIFFICULTY_CONFIGS.medium;
        }
        
        // Validate configuration integrity
        if (!this._validateDifficultyConfig(config, targetLevel)) {
            console.warn(`Invalid configuration structure for "${targetLevel}", using medium`);
            return DIFFICULTY_CONFIGS.medium;
        }
    ...
    applyToGame() {
        if (!this.game) {
            console.warn('Game instance not available, cannot apply difficulty settings');
            return;
        }
        
        if (typeof this.game.setGameSpeed !== 'function') {
            console.warn('Game instance missing setGameSpeed method, cannot apply speed settings');
            return;
        }
        ...
        try {
            ...
            if (typeof config.gameSpeed !== 'number' || config.gameSpeed <= 0 || config.gameSpeed > 1) {
                console.warn(`Invalid game speed value: ${config.gameSpeed}, using default 0.1`);
                this.game.setGameSpeed(0.1);
            } else {
                this.game.setGameSpeed(config.gameSpeed);
            }
        } catch (error) {
            console.warn('Error applying game speed settings:', error);
            // Try to set a safe default speed
            try {
                this.game.setGameSpeed(0.1);
            } catch (fallbackError) {
                console.warn('Failed to set fallback game speed:', fallbackError);
            }
        }
    }
    ...
    applyToAI() {
        if (!this.aiController) {
            console.warn('AI controller not available, cannot apply difficulty settings');
            return;
        }
        ...
        try {
            ...
            if (typeof config.turnThreshold !== 'number' || config.turnThreshold <= 0) {
                console.warn(`Invalid turnThreshold value: ${config.turnThreshold}, using default 10`);
                config.turnThreshold = 10;
            }
            
            if (typeof config.randomTurnChance !== 'number' || config.randomTurnChance < 0 || config.randomTurnChance > 1) {
                console.warn(`Invalid randomTurnChance value: ${config.randomTurnChance}, using default 0.02`);
            ...
            console.warn('Error applying AI difficulty settings:', error);
            ...
            try {
                ...
                console.warn('Failed to set fallback AI configuration:', fallbackError);
            }
        }
    }
    ...
    saveToStorage() {
        try {
            ...
            if (typeof Storage === 'undefined' || !window.localStorage) {
                console.warn('localStorage not available, cannot save difficulty setting');
                return;
            }
            
            // Validate current difficulty before saving
            if (!this.currentDifficulty || !DIFFICULTY_CONFIGS[this.currentDifficulty]) {
                console.warn(`Invalid current difficulty "${this.currentDifficulty}", cannot save`);
                return;
            }
            ...
            try {
                ...
                if (quotaError.name === 'QuotaExceededError' || quotaError.name === 'NS_ERROR_DOM_QUOTA_REACHED') {
                    console.warn('localStorage quota exceeded, cannot save difficulty setting');
                } else {
    ...
    loadFromStorage() {
        try {
            // Check if localStorage is available
            if (typeof Storage === 'undefined' || !window.localStorage) {
                console.warn('localStorage not available, using default difficulty');
                return;
            }
            ...
            try {
                ...
                console.warn('Failed to parse stored difficulty data, using default:', parseError);
            ...
            if (!data || typeof data !== 'object') {
                console.warn('Invalid stored difficulty data structure, using default');
                this._clearCorruptedStorage();
                return;
            }
            ...
            if (data.selectedDifficulty && 
                ...
                console.warn(`Invalid stored difficulty "${data.selectedDifficulty}", using default`);
                this._clearCorruptedStorage();
            }
    ...
    getAllDifficulties() {
        try {
            ...
            console.warn('Error creating difficulty configurations copy:', error);
            // Return a safe fallback
            return {
                easy: { turnThreshold: 15, randomTurnChance: 0.05, gameSpeed: 0.08, description: "Easy mode" },
                medium: { turnThreshold: 10, randomTurnChance: 0.02, gameSpeed: 0.1, description: "Medium mode" },
                hard: { turnThreshold: 8, randomTurnChance: 0.01, gameSpeed: 0.12, description: "Hard mode" }
            };
        }
    }
    ...
    _validateDifficultyConfig(config, level) {
        if (!config || typeof config !== 'object') {
            console.warn(`Configuration for "${level}" is not an object`);
            return false;
        }

        const requiredProperties = ['turnThreshold', 'randomTurnChance', 'gameSpeed', 'description'];
        for (const prop of requiredProperties) {
            if (!(prop in config)) {
                console.warn(`Configuration for "${level}" missing required property: ${prop}`);
                return false;
            }
        }
        ...
        if (typeof config.turnThreshold !== 'number' || config.turnThreshold <= 0) {
            console.warn(`Invalid turnThreshold for "${level}": ${config.turnThreshold}`);
            return false;
        }

        if (typeof config.randomTurnChance !== 'number' || 
            config.randomTurnChance < 0 || 
            config.randomTurnChance > 1) {
            console.warn(`Invalid randomTurnChance for "${level}": ${config.randomTurnChance}`);
            return false;
        }

        if (typeof config.gameSpeed !== 'number' || 
            config.gameSpeed <= 0 || 
            config.gameSpeed > 1) {
            console.warn(`Invalid gameSpeed for "${level}": ${config.gameSpeed}`);
            return false;
        }

        if (typeof config.description !== 'string' || config.description.trim().length === 0) {
            console.warn(`Invalid description for "${level}": ${config.description}`);
            return false;
        }
    ...
    validateAllConfigurations() {
        ...
        if (!allValid) {
            console.warn('Some difficulty configurations are invalid. Game may not function correctly.');
        }
