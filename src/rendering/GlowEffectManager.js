...
class GlowEffectManager {
    ...
    initialize() {
        try {
            ...
            logger.info('GlowEffectManager: Successfully initialized');
            return true;
        } catch (error) {
            logger.error('GlowEffectManager: Failed to initialize:', error);
            logger.info('GlowEffectManager: Attempting fallback mode');
            ...
            try {
                ...
                logger.error(
                    'GlowEffectManager: Fallback initialization also failed:',
                    fallbackError
                );
                this.enabled = false;
                this.fallbackMode = false;
    ...
    setIntensity(level) {
        try {
            console.log('DEBUG: setIntensity start', level);
            // Validate input
            if (!this.validateIntensityLevel(level)) {
                console.log('DEBUG: setIntensity invalid level');
            ...
            if (!this.enabled || !this.initialized) {
                console.log(
                    'DEBUG: setIntensity not enabled or initialized',
                    this.enabled,
                    this.initialized
                );
    ...
    checkParticleCompatibility() {
        try {
            ...
            if (particleSystem) {
                logger.info('GlowEffectManager: Particle system detected, ensuring compatibility');
            ...
            logger.warn('GlowEffectManager: Error checking particle compatibility:', error);
    ...
    handleGameRestart() {
        ...
        try {
            ...
            logger.info('GlowEffectManager: Game restart handled');
        } catch (error) {
            logger.error('GlowEffectManager: Error handling game restart:', error);
        }
    }
    ...
    handleGameModeSwitch(newMode, oldMode) {
        ...
        try {
            ...
            if (newMode === 'TIME_TRIAL') {
                ...
                logger.info('GlowEffectManager: Switched to Time Trial mode');
            } else if (newMode === 'ARENA_SHRINK') {
                // Arena Shrink mode might want enhanced visibility
                logger.info('GlowEffectManager: Switched to Arena Shrink mode');
            } else {
                // Classic mode
                logger.info('GlowEffectManager: Switched to Classic mode');
            }
        } catch (error) {
            logger.error('GlowEffectManager: Error handling game mode switch:', error);
        }
    }
     ...
     */
    dispose() {
        logger.info('GlowEffectManager: Starting resource cleanup');
        ...
        try {
            ...
            if (this.performanceScaler) {
                // this.performanceScaler.dispose(); // Method does not exist
                this.performanceScaler = null;
            }
            ...
            logger.info('GlowEffectManager: Resource cleanup completed');
        } catch (error) {
            logger.error('GlowEffectManager: Error during disposal:', error);
        }
    }
    ...
    startMemoryMonitoring() {
        ...
        logger.info('GlowEffectManager: Starting memory monitoring');
    ...
    checkMemoryUsage() {
        ...
        if (memoryIncrease > this.memoryStats.leakWarningThreshold) {
            logger.warn(
                `GlowEffectManager: Potential memory leak detected. Memory increased by ${Math.round(memoryIncrease / 1024 / 1024)}MB`
            );
        ...
        if (this.memoryStats.samples.length % 10 === 0) {
            logger.info(
                `GlowEffectManager: Memory usage: ${Math.round(currentMemory / 1024 / 1024)}MB (peak: ${Math.round(this.memoryStats.peakMemory / 1024 / 1024)}MB)`
            );
        }
    }
     ...
     */
    handleMemoryLeak() {
        logger.warn('GlowEffectManager: Attempting to recover from memory leak');

        try {
            // Force garbage collection if available
            if (window.gc) {
                window.gc();
                logger.info('GlowEffectManager: Forced garbage collection');
            }
            ...
            if (this.materialSystem) {
                this.materialSystem.dispose(); // Replaced clearCache with dispose
            }

            // Update memory baseline after cleanup
            setTimeout(() => {
                this.memoryStats.initialMemory = this.getMemoryUsage();
                logger.info('GlowEffectManager: Memory baseline reset after cleanup');
            }, 5000);
        } catch (error) {
            logger.error('GlowEffectManager: Error during memory leak recovery:', error);
        }
    }
    ...
    initializeLogging() {
        ...
        if (this.debugMode) {
            logger.info('GlowEffectManager: Debug mode enabled');
    ...
    logError(method, message, context = {}) {
        ...
        logger.error(`GlowEffectManager.${method}: ${message}`, context);
    }
    ...
    logWarning(method, message, context = {}) {
        ...
        this.addToLogHistory(logEntry);
        logger.warn(`GlowEffectManager.${method}: ${message}`, context);
    }
    ...
    logInfo(method, message, context = {}) {
        ...
        if (this.debugMode) {
            logger.info(`GlowEffectManager.${method}: ${message}`, context);
        }
    }
    ...
    clearLogHistory() {
        this.logHistory = [];
        logger.info('GlowEffectManager: Log history cleared');
    }
    ...
    dumpDebugState() {
        ...
        logger.info('GlowEffectManager Debug State:', state);
        return state;
    }
    ...
    attemptQualityReduction() {
        if (this.performanceScaler && this.performanceScaler.currentQuality !== 'disabled') {
    ...
    checkWebGLSupport() {
        try {
            ...
            const missingExtensions = requiredExtensions.filter(
                (ext) => !supportedExtensions.includes(ext) && !gl.getExtension(ext)
            );

            if (missingExtensions.length > 0) {
                logger.warn('GlowEffectManager: Missing WebGL extensions:', missingExtensions);
                this.showCompatibilityNotification(
            ...
            if (maxTextureSize < 1024 || maxRenderbufferSize < 1024) {
                logger.warn('GlowEffectManager: Limited texture/renderbuffer size');
                this.showCompatibilityNotification(
            ...
            if (this.isMobileGPU(renderer)) {
                logger.info('GlowEffectManager: Mobile GPU detected, using optimized settings');
                this.forceQualityMode = 'medium';
            }

            return true;
        } catch (error) {
            logger.error('GlowEffectManager: WebGL compatibility check failed:', error);
            this.showCompatibilityNotification(
    ...
    showCompatibilityNotification(title, message) {
        ...
        if (typeof document === 'undefined' || !document.body) {
            logger.warn(`GlowEffectManager Compatibility: ${title} - ${message}`);
            return;
        }
        ...
        try {
            ...
            logger.warn(`GlowEffectManager Compatibility: ${title} - ${message}`);
            logger.warn('Failed to show compatibility notification:', error.message);
        }
    }
    ...
    initializeFallbackRendering() {
        logger.info('GlowEffectManager: Initializing fallback rendering mode');
    ...
    applyIntensitySettings() {
        try {
            ...
            if (this.materialSystem) {
                try {
                    this.materialSystem.setGlobalIntensityMultiplier(config.emissive);
                } catch (error) {
