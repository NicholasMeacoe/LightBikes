...
class MusicPerformanceMonitor {
    ...
    startMonitoring() {
        ...
        logger.info('Music performance monitoring started');
    }
    ...
    stopMonitoring() {
        ...
        logger.info('Music performance monitoring stopped');
    }
    ...
    recordFadeOperation(type, duration) {
        ...
        if (recentFades > MUSIC_SYSTEM_CONFIG.MAX_CONCURRENT_FADE_OPERATIONS) {
            logger.warn('High number of concurrent fade operations detected:', {
                count: recentFades,
            });
        }
    }
    ...
    recordContextSwitch() {
        ...
        if (this.metrics.playback.contextSwitches > 5) {
            logger.warn('Frequent audio context switches detected - may impact performance');
        }
    }
    ...
    _initializeMonitoring() {
        ...
        this.memoryCleanupInterval = setInterval(() => {
            if (this._shouldPerformCleanup()) {
                logger.info('Memory cleanup recommended for music system');
            }
    ...
    _shouldPerformCleanup() {
        ...
        return (
            timeSinceLastCleanup > MUSIC_SYSTEM_CONFIG.AUDIO_BUFFER_CLEANUP_INTERVAL ||
    ...
    _checkMemoryUsage() {
        if ('memory' in performance) {
            ...
            if (memInfo.usedJSHeapSize > memInfo.jsHeapSizeLimit * 0.8) {
                logger.warn('High memory usage detected - consider cleaning up audio buffers');
            }
        }
    }
    ...
    _recordPerformanceEntry(entry) {
        ...
        if (entry.duration > 16) {
            ...
            logger.info(
                `Long-running music operation detected: ${entry.name} (${entry.duration}ms)`
            );
        }
    }
    ...
    _generateRecommendations() {
        ...
        // Fade operation recommendations
        if (this.metrics.playback.fadeOperations > 50) {
            recommendations.push({
                type: 'performance',
                priority: 'medium',
                message:
