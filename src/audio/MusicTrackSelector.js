...
const { MUSIC_TRACKS, ENERGY_LEVELS, MusicConfigUtils } = require('@/audio/MusicConfig.js');
...
class MusicTrackSelector {
    ...
    async selectTrack(trackId) {
        ...
        if (!this.trackManager.hasTrack(trackId)) {
            logger.warn(`Track not found in manager: ${trackId}`);
            return false;
        }
        ...
        try {
            ...
            logger.info(`Track selected: ${trackId}`);
            return true;
        } catch (error) {
            // Revert selection on error
            this.currentTrackId = previousTrackId;
            logger.error(`Failed to select track ${trackId}:`, { error });
            return false;
        }
    }
    ...
    getTracksByEnergy(energyLevel) {
        return this.getAvailableTracks().filter((track) => track.energyLevel === energyLevel);
    }
    ...
    isTrackAvailable(trackId) {
        ...
        return this.trackManager.hasTrack(trackId) && !this.trackManager.hasTrackError(trackId);
    }
    ...
    _validateSelection() {
        if (!this.currentTrackId || !MusicConfigUtils.isValidTrackId(this.currentTrackId)) {
            ...
            logger.warn('Invalid track selection reset to default');
        }
    }
    ...
    _notifySelectionChange(newTrackId, previousTrackId) {
        for (const callback of this.selectionCallbacks) {
            try {
                ...
                logger.error('Error in selection change callback:', { error });
            }
        }
    }
