...
class AudioManager {
    ...
    handleAudioError(error, soundType) {
        ...
        switch (errorType) {
            case 'autoplay_blocked':
                console.info('Audio blocked by autoplay policy. Audio will start after user interaction.');
                break;
                
            case 'format_unsupported':
                console.warn(`Audio format not supported for ${soundType}. Trying fallback formats.`);
                this.tryAlternativeFormat(soundType);
                break;
                
            case 'network_error':
                console.warn(`Network error loading ${soundType}. Audio will be disabled for this sound.`);
                this.markSoundUnavailable(soundType);
                break;
                
            case 'context_error':
                console.error('Audio context error. Attempting to reinitialize audio system.');
                this.handleContextError();
                break;
                
            case 'playback_error':
                console.warn(`Playback error for ${soundType}. Continuing without this sound.`);
                break;
                
            default:
                console.warn(`Unknown audio error for ${soundType}. Continuing with graceful degradation.`);
        }
    ...
    tryAlternativeFormat(soundType) {
        ...
        alternatives.forEach(async (altPath) => {
            try {
                ...
                console.log(`Successfully loaded alternative format for ${soundType}: ${altPath}`);
            } catch (altError) {
                console.warn(`Alternative format also failed for ${soundType}: ${altPath}`);
            }
        });
    }
        ...
        if (this.audioContext) {
            try {
                ...
                if (this.audioContext.state === 'closed') {
                    this.audioContext = null;
                    this.isInitialized = false;
                    console.info('Audio context closed. Will reinitialize on next audio interaction.');
                }
    ...
    async checkAutoplayPolicy() {
        ...
        try {
            ...
            console.warn('Autoplay policy may be blocking audio:', error);
            return false;
        }
    }
        ...
        try {
            ...
            if (this.audioContext.state === 'suspended') {
                try {
                    ...
                    console.warn('Failed to resume audio context, may need user interaction:', resumeError);
            ...
            if (!autoplayAllowed) {
                console.info('Autoplay may be blocked. Audio will start after user interaction.');
            }
            ...
            if (preloadSuccess) {
                ...
                console.warn('AudioManager initialization completed with some audio loading failures');
    ...
    async initializeHTMLAudio() {
        try {
            ...
            console.log('AudioManager initialized with HTML5 Audio fallback');
            return true;
    ...
    async preloadSounds() {
        ...
        if (allLoaded) {
            ...
            console.warn(`Audio preloading completed: ${successCount}/${totalSounds} sounds loaded`);
        }
    ...
    async tryFallbackFormat(soundName, originalPath) {
        try {
            ...
            console.log(`Trying fallback format for ${soundName}: ${fallbackPath}`);
            ...
            console.warn(`All formats failed for ${soundName}, audio will be disabled for this sound`);
    ...
    stopEngineSound() {
        ...
        try {
            if (this.currentEngine.stop) {
                // Web Audio API source
                this.currentEngine.stop();
            } else if (this.currentEngine.pause) {
                // HTML5 Audio element
                this.currentEngine.pause();
                this.currentEngine.currentTime = 0;
            }
    ...
    handleGameRestart() {
        ...
        try {
            ...
            if (this.musicPlayer) {
                this.musicPlayer.onGameRestart();
            }
        } catch (error) {
    ...
    performBufferCleanup() {
        try {
            ...
            this.activeSounds = this.activeSounds.filter(sound => {
                ...
                if (sound.playbackState === 'finished' || 
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('Audio Performance Metrics:', this.performanceMetrics);
            }
    ...
    async initializeMusicSystem() {
        ...
        try {
            ...
            if (musicInitSuccess) {
                ...
                console.warn('Music system initialization failed, continuing without music');
                return false;
            }
            
        } catch (error) {
            this.handleAudioError(error, 'music_initialization');
            console.warn('Music system unavailable, continuing without music');
            return false;
        }
    }
     ...
     * @returns {MusicPlayer|null} Music player instance or null if not initialized
     */
    getMusicPlayer() {
     ...
     * @returns {MusicSettings|null} Music settings instance or null if not initialized
     */
    getMusicSettings() {
