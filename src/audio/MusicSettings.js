...
class MusicSettings {
    ...
    updateSettings(newSettings) {
        ...
        for (const [key, value] of Object.entries(newSettings)) {
            if (!this.schema[key]) {
                logger.warn(`Unknown music setting: ${key}`);
                continue;
            }

            if (!this._validateSetting(key, value)) {
                logger.warn(`Invalid value for music setting ${key}:`, { value });
                return false;
            }
    ...
    save() {
        try {
            ...
            logger.warn('Failed to save music settings to localStorage:', { error });
            return false;
        }
    }
    ...
    load() {
        try {
            ...
            logger.warn('Failed to load music settings from localStorage:', { error });
    ...
    _migrateSettings(loadedData) {
        ...
        if (!loadedData.version && !loadedData.settings) {
            logger.info('Migrating legacy music settings format');
    ...
    clearStorage() {
        try {
            ...
            logger.warn('Failed to clear music settings from localStorage:', { error });
            return false;
        }
    }
    ...
    _mergeWithDefaults(loadedSettings) {
        ...
        for (const [key, value] of Object.entries(loadedSettings)) {
            if (this.schema[key] && this._validateSetting(key, value)) {
                ...
                logger.warn(`Invalid music setting ${key} loaded from storage, using default:`, {
                    value,
                });
            }
        }
