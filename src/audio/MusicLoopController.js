...
class MusicLoopController {
    ...
    async startLoop(track, destination, startTime = 0) {
        ...
        logger.info(`Started seamless loop for track: ${track.getId()}`);
    }
    ...
    async stopLoop(fadeOutDuration = 0) {
        ...
        logger.info('Seamless loop stopped');
    }

    /**
     * Create a loop source with precise timing
     * @param {any} track - Track to create source for
    ...
    _scheduleNextLoop(track, nextStartTime) {
        ...
        if (scheduleDelay > 0) {
            setTimeout(
                ...
                Math.max(0, scheduleDelay - 50)
            ); // Schedule 50ms early for precision
        }
    }
    ...
    _analyzeLoopPoints(audioBuffer) {
        ...
        if (sampleDifference > 0.1) {
            ...
            this.crossfadeDuration = Math.min(
                0.05,
                (this.loopEndTime - this.loopStartTime) * 0.001
            );
            logger.info(`Loop gap detected, enabling ${this.crossfadeDuration * 1000}ms crossfade`);
        }
    }
     ...
     * @param {any} track - Track to restart
