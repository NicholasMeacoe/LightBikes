...
describe('CustomizationManager', () => {
    ...
    describe('error handling', () => {
        ...
        it('should apply preview changes permanently', () => {
            ...
            expect(customizationManager.previewMode).toBe(false);
            expect(customizationManager.currentState.bikeColor).toBe('#FF0000');
            expect(mockPreferenceStorage.savePreferences).toHaveBeenCalled();
        });
    ...
    describe('preferences management', () => {
        ...
        it('should load saved preferences', () => {
            ...
            const newManager = new CustomizationManager(mockRenderingEngine, mockPreferenceStorage);
            
            expect(newManager.getCurrentState().bikeColor).toBe('#0000FF');
            expect(newManager.getCurrentState().trailColor).toBe('#FF0000');
            expect(newManager.getCurrentState().trailStyle).toBe('glowing');
            expect(newManager.getCurrentState().arenaTheme).toBe('space');
        });
    ...
    describe('color validation', () => {
        it('should validate correct hex colors', () => {
            expect(customizationManager.validateColor('#FF0000')).toBe(true);
            expect(customizationManager.validateColor('#00ff00')).toBe(true);
            expect(customizationManager.validateColor('#123ABC')).toBe(true);
        });
        
        it('should reject invalid color formats', () => {
            expect(customizationManager.validateColor('FF0000')).toBe(false);  // Missing #
            expect(customizationManager.validateColor('#FF00')).toBe(false);   // Too short
            expect(customizationManager.validateColor('#GGGGGG')).toBe(false); // Invalid hex
            expect(customizationManager.validateColor('red')).toBe(false);     // Color name
            expect(customizationManager.validateColor('')).toBe(false);        // Empty
            expect(customizationManager.validateColor(null)).toBe(false);      // Null
        });
    });
    
    describe('color contrast validation', () => {
        it('should validate sufficient color contrast', () => {
            // White on dark blue should have good contrast
            expect(customizationManager.validateColorContrast('#FFFFFF', '#000033')).toBe(true);
            
            // Bright colors on dark background should have good contrast
            expect(customizationManager.validateColorContrast('#FF0000', '#000033')).toBe(true);
            expect(customizationManager.validateColorContrast('#00FF00', '#000033')).toBe(true);
        });
        
        it('should reject insufficient color contrast', () => {
            // Dark colors on dark background should have poor contrast
            expect(customizationManager.validateColorContrast('#000000', '#000033')).toBe(false);
            expect(customizationManager.validateColorContrast('#333333', '#000033')).toBe(false);
        });
    });
    
    describe('utility methods', () => {
        it('should return color presets', () => {
            const presets = customizationManager.getColorPresets();
