...
class MusicErrorHandler {
    ...
    async handleLoadingError(error, trackId, originalUrl, attempt = 0) {
        ...
        switch (strategy.type) {
            ...
            case 'graceful_degradation':
                return this._handleGracefulDegradation(trackId, errorInfo);
    ...
    getUserFriendlyMessage(trackId, errorType) {
        const messages = {
            ...
            [ERROR_TYPES.NETWORK_ERROR]: 'Network connection issue. Music will be available when connection is restored.',
            ...
            [ERROR_TYPES.CONTEXT_ERROR]: 'Audio system initialization failed. Some features may be unavailable.',
            [ERROR_TYPES.PLAYBACK_FAILED]: `Playback error for "${trackId}". Continuing without background music.`,
            [ERROR_TYPES.SETTINGS_ERROR]: 'Music settings could not be saved. Changes will be temporary.',
            [ERROR_TYPES.UNKNOWN_ERROR]: 'An unexpected audio error occurred. Continuing without background music.'
        };
    ...
    shouldAttemptRecovery(trackId, errorType) {
        ...
        const maxRetries = MUSIC_SYSTEM_CONFIG.RETRY_ATTEMPTS;
    ...
    async _handleFormatFallback(trackId, originalUrl, strategy) {
        ...
        if (fallbackUrl && fallbackUrl !== originalUrl) {
            ...
            return this._handleGracefulDegradation(trackId, { type: ERROR_TYPES.FORMAT_UNSUPPORTED });
        }
    }
     ...
     * @private
     */
    _handleGracefulDegradation(trackId, errorInfo) {
        return {
            action: 'degrade',
            trackId: trackId,
            message: this.getUserFriendlyMessage(trackId, errorInfo.type),
            continueWithoutMusic: true,
            reason: 'Graceful degradation - continue without music'
        };
    }
    ...
    _logError(errorInfo, trackId, attempt = 0) {
        ...
        console.warn(`Music system error [${errorInfo.type}] for track ${trackId}:`, errorInfo.message);
    }
