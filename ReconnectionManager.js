...
class ReconnectionManager {
    ...
    handleDisconnect(data) {
        console.log('[ReconnectionManager] Disconnected:', data.reason);
    ...
    handleReconnecting(data) {
        console.log('[ReconnectionManager] Reconnecting, attempt:', data.attemptNumber);
    ...
    handleReconnected(data) {
        console.log('[ReconnectionManager] Reconnected after', data.attemptNumber, 'attempts');
    ...
    attemptReconnect() {
        ...
        console.log(`[ReconnectionManager] Reconnection attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts}`);
     ...
     */
    abandonReconnection() {
        console.log('[ReconnectionManager] Reconnection abandoned after', this.reconnectAttempts, 'attempts');
    ...
    preserveState() {
        ...
        console.log('[ReconnectionManager] State preserved:', this.preservedState);
    }
    ...
    async restoreState() {
        if (!this.preservedState.wasInGame || !this.preservedState.roomId) {
            console.log('[ReconnectionManager] No state to restore');
            return;
        }
        
        console.log('[ReconnectionManager] Attempting to restore state:', this.preservedState);
        ...
        try {
            ...
            if (disconnectDuration > 30000) {
                console.log('[ReconnectionManager] Disconnected too long, cannot rejoin');
            ...
            if (response.success) {
                console.log('[ReconnectionManager] State restored successfully');
                this.triggerEvent('stateRestored', {
                    roomId: this.preservedState.roomId,
                    gameState: response.gameState
                });
            } else {
                console.log('[ReconnectionManager] Failed to restore state:', response.error);
                this.triggerEvent('reconnectFailed', {
                    reason: response.error || 'unknown'
                });
            }
        } catch (error) {
            console.error('[ReconnectionManager] Error restoring state:', error);
            this.triggerEvent('reconnectFailed', {
    ...
    updateConnectionQuality(quality) {
        if (this.connectionQuality !== quality) {
            ...
            console.log(`[ReconnectionManager] Connection quality changed: ${previousQuality} -> ${quality}`);
     ...
     */
    forceReconnect() {
        console.log('[ReconnectionManager] Forcing reconnection');
     ...
     */
    cancelReconnection() {
        console.log('[ReconnectionManager] Cancelling reconnection');
