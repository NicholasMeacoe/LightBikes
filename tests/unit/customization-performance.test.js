...
describe('Customization System Performance', () => {
    ...
    beforeEach(() => {
        ...
        mockRenderingEngine = {
            ...
            emissiveMaterialSystem: {
                ...
                createBikeMaterial: jest.fn().mockReturnValue(new THREE.MeshLambertMaterial({ color: 0x00FF00 })),
                createTrailMaterial: jest.fn().mockReturnValue(new THREE.MeshLambertMaterial({ color: 0x00FF00 }))
            },
    ...
    describe('material reuse performance', () => {
        it('should reuse materials efficiently', () => {
            ...
            // Create many materials with same properties
            const materials = [];
            ...
            // All materials should be the same instance (reused)
            const uniqueMaterials = new Set(materials);
            expect(uniqueMaterials.size).toBe(1);
            
            // Should only create one material despite 100 requests
    ...
    describe('frame rate impact', () => {
        it('should maintain target frame rate with default settings', () => {
            ...
            mockTime = 1016; // 16ms later
            performanceOptimizer.updatePerformanceMetrics();
            
            const metrics = performanceOptimizer.getPerformanceMetrics();
            expect(metrics.frameTime).toBeLessThanOrEqual(targetFrameTime * 1.2); // 20% tolerance
        ...
        it('should detect performance degradation', () => {
            ...
            mockTime = 1030; // 30ms later (33 FPS)
            performanceOptimizer.updatePerformanceMetrics();
            
            const metrics = performanceOptimizer.getPerformanceMetrics();
            expect(metrics.frameTime).toBeGreaterThan(16.67 * 1.2);
    ...
    describe('memory usage optimization', () => {
        ...
        it('should handle texture pooling efficiently', () => {
            ...
            const texture1 = performanceOptimizer.getOrCreateTexture('gradient', 'test-key', createTextureFn);
            const texture2 = performanceOptimizer.getOrCreateTexture('gradient', 'test-key', createTextureFn);
