...
describe('MusicErrorHandler', () => {
    ...
    describe('playback error handling', () => {
        it('should handle recoverable playback errors', () => {
            ...
            const recovery = errorHandler.handlePlaybackError(contextError, 'play', {
                trackId: 'test-track',
            });
        ...
        it('should handle non-recoverable playback errors', () => {
            ...
            const recovery = errorHandler.handlePlaybackError(fatalError, 'play', {
                trackId: 'test-track',
            });
    ...
    describe('recovery decision making', () => {
        it('should recommend recovery for network errors within retry limit', () => {
            const shouldRecover = errorHandler.shouldAttemptRecovery(
                'test-track',
                ERROR_TYPES.NETWORK_ERROR
            );
        ...
        it('should not recommend recovery for non-recoverable errors', () => {
            const shouldRecover = errorHandler.shouldAttemptRecovery(
                'test-track',
                ERROR_TYPES.FORMAT_UNSUPPORTED
            );
    ...
    describe('error statistics', () => {
        it('should track error statistics correctly', () => {
            ...
            errorHandler._logError(
                { type: ERROR_TYPES.NETWORK_ERROR, message: 'Network failed' },
                'track1'
            );
            errorHandler._logError(
                { type: ERROR_TYPES.LOADING_FAILED, message: 'Load failed' },
                'track2'
            );
            errorHandler._logError(
                { type: ERROR_TYPES.NETWORK_ERROR, message: 'Network failed again' },
                'track1'
            );
        ...
        it('should limit error log size', () => {
            ...
            for (let i = 0; i < 150; i++) {
                errorHandler._logError(
                    { type: ERROR_TYPES.UNKNOWN_ERROR, message: `Error ${i}` },
                    `track${i}`
                );
            }
